---
name: dividend-article-writer
description: >-
  为 Dividend Guide 网站创建符合 Google E-E-A-T 标准的 SEO 科普文章。
  每次处理一个关键词，输出英文文章。涵盖选题判断、文章结构模板、
  EEAT 硬约束、内部/外部链接策略、封面图处理和构建验证。
  当用户要求"根据关键词写文章"、"针对XX写一篇科普"、"write an article for keyword XX"时触发。
  在 Dividend Guide 项目目录下工作时使用。
version: 1.0.0
author: Dividend Guide Operations Team
license: Proprietary (Dividend Guide)
metadata:
  hermes:
    tags: [dividend, content, eeat, seo, article, finance]
    related_skills:
      - seo-content
...
**这是最关键的一步。** 内容草稿完成后立即执行，无需手动干预。

**工作流：**

1. **加载 `seo-content-iterative` skill** — 使用 `skill` 工具加载 `seo-content-iterative` skill

2. **执行 5 轮审核** — 按照 `seo-content-iterative` 的指令，对文章文件执行完整的 5 轮"审计→定位问题→修改→再审计"循环。每一轮审计范围包括：
   - E-E-A-T 四维度（Experience / Expertise / Authoritativeness / Trustworthiness）
   - AI 内容检测
   - AI Citation Readiness（GEO 信号）
   - 内容结构指标（层级、段落、内外链、FAQ 等）
   - 每轮处理 top 3-5 个问题，优先 Critical/Major
   - 跳过不可变字段（author、image、draft、日期）

3. **自动清理** — 审核完成后移除变更日志注释块

**重要规则：**
- 不可变字段（author、image 路径、draft、pubDate、modDate）跳过不碰
- 所有数据点不得被修改或删除
...
- [ ] 虚构的个人作者名已移除
- [ ] **seo-content-iterative 5 轮审核完成**：无遗留 Critical/Major 问题
